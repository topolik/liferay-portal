...
public class PortletURLImplTest {
	...
	public void testGenerateToStringWithAutopropagating() throws Exception {
		...
		for (String autoParam : autopropagatedParameters) {
			String namespacedAutoParam = "_" + PORTLET_DDM + "_" + autoParam;

			if ((counter++ % 2) == 0) {
				request.setParameter(autoParam, new String[]{"0", "1"});
			}
			else {
				request.setParameter(
					namespacedAutoParam, new String[]{"0", "1"});
			}

			expectedURLParts.add(namespacedAutoParam + "=0");
			expectedURLParts.add(namespacedAutoParam + "=1");
		}

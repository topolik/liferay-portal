 ...
 */
@ProviderType
public class EmailOTPPersistenceImpl
	extends BasePersistenceImpl<EmailOTP> implements EmailOTPPersistence {

	/*
	 * NOTE FOR DEVELOPERS:
	 *
	 * Never modify or reference this class directly. Always use <code>EmailOTPUtil</code> to access the email otp persistence. Modify <code>service.xml</code> and rerun ServiceBuilder to regenerate this class.
	 */
	public static final String FINDER_CLASS_NAME_ENTITY =
		EmailOTPImpl.class.getName();
	...
	public void cacheResult(EmailOTP emailOTP) {
		entityCache.putResult(
			EmailOTPModelImpl.ENTITY_CACHE_ENABLED, EmailOTPImpl.class,
			emailOTP.getPrimaryKey(), emailOTP);

		finderCache.putResult(
			_finderPathFetchByUserId, new Object[] {emailOTP.getUserId()},
			emailOTP);
	...
	protected void cacheUniqueFindersCache(
		EmailOTPModelImpl emailOTPModelImpl) {
		...
		finderCache.putResult(
			_finderPathFetchByUserId, args, emailOTPModelImpl, false);
	}
	...
	protected void clearUniqueFindersCache(
		...
		if ((emailOTPModelImpl.getColumnBitmask() &
			...
			Object[] args = new Object[] {
				emailOTPModelImpl.getOriginalUserId()
			};
	...
	public EmailOTP remove(Serializable primaryKey)
		...
		try {
			...
			EmailOTP emailOTP = (EmailOTP)session.get(
				EmailOTPImpl.class, primaryKey);
	...
	protected EmailOTP removeImpl(EmailOTP emailOTP) {
		...
		try {
			...
			if (!session.contains(emailOTP)) {
				emailOTP = (EmailOTP)session.get(
					EmailOTPImpl.class, emailOTP.getPrimaryKeyObj());
			}
	...
	public EmailOTP updateImpl(EmailOTP emailOTP) {
		...
		entityCache.putResult(
			EmailOTPModelImpl.ENTITY_CACHE_ENABLED, EmailOTPImpl.class,
			emailOTP.getPrimaryKey(), emailOTP, false);
	...
	/**
	 * Returns the email otp with the primary key or throws a <code>com.liferay.portal.kernel.exception.NoSuchModelException</code> if it could not be found.
	 *
	 * @param primaryKey the primary key of the email otp
	...
	@Override
	public List<EmailOTP> findAll(
		int start, int end, OrderByComparator<EmailOTP> orderByComparator) {
	...
	public List<EmailOTP> findAll(
		...
		if (retrieveFromCache) {
			list = (List<EmailOTP>)finderCache.getResult(
		...
		if (list == null) {
			...
			try {
				...
				if (!pagination) {
					list = (List<EmailOTP>)QueryUtil.list(
	...
	public void afterPropertiesSet() {
		_finderPathWithPaginationFindAll = new FinderPath(
			EmailOTPModelImpl.ENTITY_CACHE_ENABLED,
	...
	private static final String _SQL_SELECT_EMAILOTP =
		"SELECT emailOTP FROM EmailOTP emailOTP";

	private static final String _SQL_SELECT_EMAILOTP_WHERE =
		"SELECT emailOTP FROM EmailOTP emailOTP WHERE ";

	private static final String _SQL_COUNT_EMAILOTP =
		"SELECT COUNT(emailOTP) FROM EmailOTP emailOTP";

	private static final String _SQL_COUNT_EMAILOTP_WHERE =
		"SELECT COUNT(emailOTP) FROM EmailOTP emailOTP WHERE ";

	private static final String _ORDER_BY_ENTITY_ALIAS = "emailOTP.";

	private static final String _NO_SUCH_ENTITY_WITH_PRIMARY_KEY =
		"No EmailOTP exists with the primary key ";

	private static final String _NO_SUCH_ENTITY_WITH_KEY =
		"No EmailOTP exists with the key {";

	private static final Log _log = LogFactoryUtil.getLog(
		EmailOTPPersistenceImpl.class);

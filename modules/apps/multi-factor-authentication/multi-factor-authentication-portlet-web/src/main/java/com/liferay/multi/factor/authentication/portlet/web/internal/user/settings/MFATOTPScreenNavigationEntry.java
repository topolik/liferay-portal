...
public class MFATOTPScreenNavigationEntry
	...
	public void render(HttpServletRequest request, HttpServletResponse response)
		...
		List<MFAVerifier> mfaVerifiers = _mfaRegistry.getMFAVerifiers();

		/*List<MFAVerifier> mfaVerifiers = _mfaRegistry.getMFAVerifiers();

		mfaVerifiers.stream().anyMatch(verifier -> verifier instanceof TOTPVERI)

		BrowserMFAVerifier mfa = (BrowserMFAVerifier)mfaVerifiers.get(0);

		long userId = PortalUtil.getUserId(request);

		mfa.includeSetup(userId, request, response);*/
		...
		_jspRenderer.renderJSP(
			_setServletContext, request, response, "/totp_setup.jsp");
	}

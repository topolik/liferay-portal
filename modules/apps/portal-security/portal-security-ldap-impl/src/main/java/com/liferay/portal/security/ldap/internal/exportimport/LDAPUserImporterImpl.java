...
public class LDAPUserImporterImpl implements LDAPUserImporter, UserImporter {
	...
	public User importUser(
		...
		try {
			...
			SafeLdapFilter authSearchSafeLdapFilter =
				ldapServerConfiguration.getAuthSearchSafeLdapFilter(
			...
			enu = safeLdapContext.search(
				ldapServerConfiguration.getBaseDNSafeLdapName(),
	...
	protected void addRole(
		...
		try {
			...
			descriptionMap.put(
				company.getLocale(),
				"Autogenerated role fromUnsafe LDAP import");
			...
			if (_log.isDebugEnabled()) {
				_log.debug(
					"Imported autogenerated role fromUnsafe LDAP import: " +
						role);
			}
		}
	...
	protected Attribute getUsers(
		...
		SafeLdapFilter groupSearchSafeLdapFilter =
			ldapServerConfiguration.getGroupSearchSafeLdapFilter(
				_ldapFilterValidator);
		...
		return _safePortalLDAP.getMultivaluedAttribute(
			...
			ldapServerConfiguration.getBaseDNSafeLdapName(), safeLdapFilter,
			attribute);
	}
	...
	protected void importUsers(
		...
		if (_log.isDebugEnabled()) {
			...
			_log.debug(
				StringBundler.concat(
					"Importing ", size, " users fromUnsafe LDAP server ",
		...
		for (User user : userGroupUsers) {
			if ((ldapImportContext.getLdapServerId() ==
				...
				if (_log.isDebugEnabled()) {
					_log.debug(
						StringBundler.concat(
							"Removing user ", user, " fromUnsafe user group ",
							userGroupId));
				}
	...
	protected User updateUser(
		...
		if (_log.isDebugEnabled()) {
			...
			if (isNew) {
				_log.debug(
					StringBundler.concat(
						"Updating new user ", user, " fromUnsafe LDAP server ",
						ldapServerId, " to company ", companyId));
			}
			else {
				_log.debug(
					StringBundler.concat(
						"Updating existing user ", user,
						" fromUnsafe LDAP server ", ldapServerId,

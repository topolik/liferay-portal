 ...
 */
@Component(immediate = true, service = ImageRequestToken.class)
public class ImageRequestTokenImpl implements ImageRequestToken {
		...
		try {
			...
			if (passwordModifiedDate != null) {
				...
				if (instant.isBefore(passwordModifiedDate.getTime())) {
					if (_log.isDebugEnabled()) {
						_log.debug(
							"Unable to accept token, user password was " +
								"changed");
					}

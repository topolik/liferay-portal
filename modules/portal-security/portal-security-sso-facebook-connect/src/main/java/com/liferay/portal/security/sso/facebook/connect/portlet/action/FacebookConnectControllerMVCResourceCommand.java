...
@Component(
	...
	property = {
		...
		"mvc.command.name=/login/facebook_connect_oauth" + PortletKeys.FAST_LOGIN
	},
	service = MVCResourceCommand.class
)
public class FacebookConnectControllerMVCResourceCommand
	...
	public String getRedirectURI(
		...
		selfResourceURL.setParameter(
			"redirect", request.getParameter("redirect"));

		String redirect_uri = HttpUtil.addParameter(
			facebookAuthRedirectURL, "postAuthRedirect",
			selfResourceURL.toString());
	...
	private String getFacebookAuthURL(
		...
		String loginRedirectURL = getRedirectURI(
			request, response, themeDisplay);

		facebookAuthURL = HttpUtil.addParameter(
			facebookAuthURL, "client_id", facebookAppId);
		facebookAuthURL = HttpUtil.addParameter(
			facebookAuthURL, "redirect_uri", loginRedirectURL.toString());
		facebookAuthURL = HttpUtil.addParameter(
			facebookAuthURL, "scope", "email");
		facebookAuthURL = HttpUtil.addParameter(
			facebookAuthURL, "state", getCurrentCSRFToken(request));
	...
	private boolean handleAlreadySignedIn(
		ResourceRequest request, ResourceResponse response,
		ThemeDisplay themeDisplay) {
	...
	private boolean handleIncompleteUser(
			ResourceRequest request, ResourceResponse response)
		throws PortletException {
